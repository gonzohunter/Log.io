  ...
  dom.find(".screens .stream").click(function() {
    if ($(this).is(":checked")) {
      try {
        ...
        dom.find(".status").addClass("statuscolor" + log_file.color);
      } catch(err) {
        $(this).attr("checked","");
        alert(err);
      }
...
Stream.prototype.log = function(log_file, msg) {
  if (!this._paused) {
    ...
    console = this._dom.find(".console")
    console.append(html).scrollTop(console[0].scrollHeight);
  }
